  ...
  return (
    <main className="min-h-screen bg-base px-4 py-8 text-ink sm:px-6 sm:py-12">
      <div className="mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-[460px] flex-col items-center justify-center">
        <Link href="/" aria-label="MAX home" className="mb-8 inline-flex items-center">
          <img src="/logo.png" alt="MAX AI" className="h-11 w-11 rounded-xl object-contain" />
        </Link>
          ...
          {step === "choose" ? (
            <div>
              <div className="text-center">
                <h1 className="font-display text-[28px] font-semibold tracking-[-0.03em] text-ink sm:text-[30px]">Choose a MAX Account</h1>
                <p className="mx-auto mt-2 max-w-[350px] text-sm leading-6 text-ink-muted">Sign in with your MAX Account to continue to MAX services and apps.</p>
              </div>

              <div className="mt-8 space-y-3">
                {rememberedAccount ? (
                  <button type="button" onClick={() => beginWithAccount(rememberedAccount)} className="group flex w-full items-center gap-3 rounded-2xl border border-glass-border bg-base px-4 py-3.5 text-left transition-colors hover:bg-base-raised-strong">
                    <div className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-brand-100 p-2 dark:bg-brand-500/15"><img src="/logo.png" alt="" className="h-full w-full rounded-full object-contain" /></div>
                    <div className="min-w-0 flex-1"><p className="text-xs font-medium text-ink-faint">Last used</p><p className="mt-0.5 truncate text-sm font-semibold text-ink">{rememberedAccount}</p></div>
                    <span className="text-sm font-semibold text-brand-600 transition-transform group-hover:translate-x-0.5 dark:text-brand-400">Continue</span>
                  </button>
                ) : (
                  <button type="button" onClick={useAnotherAccount} className="flex w-full items-center gap-3 rounded-2xl border border-glass-border bg-base px-4 py-3.5 text-left transition-colors hover:bg-base-raised-strong">
                    <div className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-brand-100 dark:bg-brand-500/15"><UserRound className="h-5 w-5 text-brand-700 dark:text-brand-300" /></div>
                    <div className="min-w-0 flex-1"><p className="text-sm font-semibold text-ink">Use your MAX Account</p><p className="mt-0.5 text-sm text-ink-muted">Enter your email or username</p></div>
                    <span className="text-lg text-ink-faint">›</span>
                  </button>
                )}

                {rememberedAccount && (
                  <button type="button" onClick={useAnotherAccount} className="flex w-full items-center justify-center gap-2 rounded-2xl border border-transparent px-4 py-3 text-sm font-semibold text-brand-600 transition-colors hover:bg-brand-50 dark:text-brand-400 dark:hover:bg-brand-500/10">
                    <UserRound className="h-4 w-4" />
                    Use another MAX Account
                  </button>
                )}
              </div>
                ...
                <div>
                  <label className="block">
                    ...
                    <span className="relative block">
                      ...
                      <input type="password" autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} className="h-12 w-full rounded-xl border border-glass-border bg-base px-11 text-sm text-ink outline-none transition-colors placeholder:text-ink-faint focus:border-brand-500 focus:ring-2 focus:ring-brand-500/15" placeholder="Password" required autoFocus={!!identifier} />
                    </span>
                  </label>
                  <div className="mt-3 flex items-center justify-between gap-4">
                    <label className="flex items-center gap-2 text-sm text-ink-muted"><input type="checkbox" checked={rememberMe} onChange={(event) => setRememberMe(event.target.checked)} className="h-4 w-4 rounded border-glass-border bg-base text-brand-600 focus:ring-brand-500" />Remember this account</label>
                    <Link href="/forgot-password" className="text-sm font-medium text-brand-600 hover:underline dark:text-brand-400">Forgot password?</Link>
                  </div>
                </div>
