...
export default function DeveloperPage() {
  ...
  async function createClient() {
    ...
    try {
      setCreating(true);
      const result = await oauthApi.createClient({ name: appName, redirectUris: [redirect], scopes, isConfidential: false });
  ...
  return <AuthFeatureShell eyebrow="MAX Developers" title="Build with MAX Identity" description="Register OAuth applications, manage credentials and prepare your products for Continue with MAX AI.">
    <div className="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-glass-border bg-base-raised p-4">
      <div><p className="font-semibold text-ink">MAX Developer Platform</p><p className="mt-1 text-sm text-ink-muted">Manage MAX Identity integrations from the dedicated developer portal.</p></div>
      <a href="https://developers.max-ai.name.ng" className="inline-flex items-center gap-2 rounded-xl bg-brand-600 px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-brand-700"><ExternalLink className="h-4 w-4" /> Open Developer Platform</a>
    </div>
    ...
    <div className="grid gap-5 lg:grid-cols-[1.35fr_.65fr]">
      <GlassCard>
        ...
        <button disabled={creating} onClick={createClient} className="mt-2 inline-flex items-center gap-2 rounded-xl bg-brand-500 px-4 py-3 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50"><Plus className="h-4 w-4" /> {creating ? "Registering…" : "Register public PKCE application"}</button>
        {secret && <div className="mt-4 rounded-2xl border border-success/20 bg-success/5 p-4"><p className="flex items-center gap-2 text-sm font-medium text-success"><CheckCircle2 className="h-4 w-4" /> Client secret created</p><p className="mt-1 text-xs text-ink-muted">This client is public PKCE, so the browser does not need a secret. Keep any confidential client secrets server-side.</p><div className="mt-3 flex items-center gap-2"><code className="min-w-0 flex-1 overflow-hidden text-ellipsis rounded-xl bg-black/20 px-3 py-2 text-xs">{showSecret ? secret : "•".repeat(Math.min(secret.length, 36))}</code><button onClick={() => setShowSecret((v) => !v)} className="rounded-lg p-2 text-ink-muted hover:text-ink" aria-label="Toggle secret visibility">{showSecret ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button><button onClick={() => copy(secret)} className="rounded-lg p-2 text-ink-muted hover:text-ink" aria-label="Copy client secret"><Copy className="h-4 w-4" /></button></div><button onClick={() => setSecret(null)} className="mt-2 text-xs text-ink-faint hover:text-ink">Dismiss secret</button></div>}
      </GlassCard>
      <div className="space-y-5"><GlassCard><h2 className="font-display text-lg font-semibold">Integration</h2><p className="mt-2 text-sm leading-6 text-ink-muted">Use OAuth 2.0 Authorization Code with S256 PKCE for browser and mobile public clients. MAX Auth remains the universal identity provider.</p><div className="mt-4 rounded-xl border border-white/10 bg-black/20 p-3 font-mono text-xs text-ink-muted">https://api.max-ai.name.ng/authorize</div><button onClick={() => copy("Continue with MAX AI")} className="mt-3 inline-flex items-center gap-2 text-sm font-medium text-brand-300 hover:text-brand-200"><Copy className="h-4 w-4" /> {copied ? "Copied" : "Copy integration label"}</button></GlassCard><GlassCard><div className="flex items-center gap-3"><Shield className="h-5 w-5 text-success" /><div><p className="font-medium">Universal MAX identity</p><p className="text-xs text-ink-muted">Existing MAX users authenticate through the same MAX Auth backend across MAX products and developer applications.</p></div></div></GlassCard></div>
    </div>
  </AuthFeatureShell>;
}
