  ...
  return <AuthFeatureShell eyebrow="MAX Developers" title="Build with MAX Identity" description="Register OAuth applications, manage credentials and prepare your products for Continue with MAX AI.">
    <div className="grid gap-5 lg:grid-cols-[1.35fr_.65fr]">
      ...
      <div className="space-y-5">
        <GlassCard>
          <h2 className="font-display text-lg font-semibold">OAuth configuration</h2>
          <p className="mt-2 text-sm leading-6 text-ink-muted">Use OAuth 2.0 Authorization Code with S256 PKCE for browser and mobile public clients. MAX Auth remains the universal identity provider.</p>
          <div className="mt-4 rounded-xl border border-white/10 bg-black/20 p-3 font-mono text-xs text-ink-muted">https://api.max-ai.name.ng/authorize</div>
          <button onClick={() => copy("Continue with MAX AI")} className="mt-3 inline-flex items-center gap-2 text-sm font-medium text-brand-300 hover:text-brand-200"><Copy className="h-4 w-4" /> {copied ? "Copied" : "Copy integration label"}</button>
          <a href="https://developers.max-ai.name.ng" className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl border border-brand-500/30 bg-brand-500/10 px-4 py-3 text-sm font-semibold text-brand-300 transition-colors hover:bg-brand-500/15 hover:text-brand-200"><ExternalLink className="h-4 w-4" /> Go to the MAX AI Developers platform</a>
        </GlassCard>

        <GlassCard><div className="flex items-center gap-3"><Shield className="h-5 w-5 text-success" /><div><p className="font-medium">Universal MAX identity</p><p className="text-xs text-ink-muted">Existing MAX users authenticate through the same MAX Auth backend across MAX products and developer applications.</p></div></div></GlassCard>
      </div>
    </div>
  </AuthFeatureShell>;
}
