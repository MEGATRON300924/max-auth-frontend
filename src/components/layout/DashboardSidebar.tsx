...
import { LayoutDashboard, User, Sparkles, ShieldCheck, Laptop, History, KeyRound, Link2, Settings, Code2, Fingerprint, LockKeyhole, LifeBuoy, Database } from "lucide-react";
...
export function DashboardSidebar({ className }: { className?: string }) {
  ...
  return <aside className={cn("flex h-full w-64 shrink-0 flex-col border-r border-glass-border bg-base-raised/60 backdrop-blur-xl", className)}>
    ...
    <nav className="flex-1 space-y-1 overflow-y-auto px-3 py-4" aria-label="Dashboard">
      {nav.map(({ href, label, icon: Icon }) => { const active = pathname === href; return <Link key={href} href={href} aria-current={active ? "page" : undefined} className={cn("flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all duration-150", active ? "bg-brand-500/15 text-brand-300 shadow-[inset_0_0_0_1px_rgba(59,130,246,0.3)]" : "text-ink-muted hover:bg-white/5 hover:text-ink")}><Icon className="h-4.5 w-4.5 shrink-0" aria-hidden="true" />{label}</Link> })}
    </nav>
